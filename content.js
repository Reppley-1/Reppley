...
// --- ✨ [수정] 캐릭터 액션 UI 생성 함수 (수정 시 새로운 필드 불러오기) ---
  function createCharacterActionUI() {
    ...
    playBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (character.is_public) {
                ...
                if (tempDecrypted === null) { alert('비밀번호가 틀렸습니다.'); return; }
    ...
    editBtn.addEventListener('click', async () => {
        ...
        try {
            // ✨ [수정] 불러올 컬럼에 first_message, start_setting 추가
            const { data: character, error } = await supabase.from('characters').select('*, first_message, start_setting').eq('id', characterId).single();
            if (error || !character) throw new Error('DB에서 캐릭터를 찾을 수 없습니다.');
            ...
            // ✨ [수정] first_message와 start_setting 필드 값 채우기
            // first_message는 jsonb이므로, greeting 키의 값을 추출해서 넣습니다.
            const firstMessageText = (character.first_message && character.first_message.greeting) ? character.first_message.greeting : '';
            document.getElementById('creator-first-message').value = firstMessageText;
            document.getElementById('creator-start-setting').value = character.start_setting || '';
            ...
            creatorModal.dataset.editPassword = password;
...
// --- ✨ [대폭 수정] 캐릭터 생성/수정 UI 함수 (첫 인사말(jsonb), 시작 설정 필드 추가 및 저장 로직 수정) ---
  function createCharacterCreatorUI() {
        ...
        <div class="reppley-persona-modal-body">
            <div class="creator-scrollable-content">
                ...
                <div id="tab-detailed-settings" class="creator-tab-content">
                    <div class="creator-form-section">
                        <label>첫 인사말</label>
                        <textarea id="creator-first-message" placeholder="캐릭터 플레이 시작 시, 채팅방에 가장 먼저 표시될 메시지입니다. (마크다운 서식 지원)"></textarea>
                        <div class="description">채팅이 시작되면 사라지는 임시 환영 메시지입니다. 비워두면 표시되지 않습니다.</div>
                    </div>
                    <div class="creator-form-section">
                        <label>시작 설정</label>
                        <textarea id="creator-start-setting" placeholder="AI가 첫 대답을 생성할 때만 참고할 숨겨진 지침입니다. (예: 특정 상황, 장소, 시간대 설정)"></textarea>
                        <div class="description">이 내용은 사용자에게 보이지 않으며, AI의 첫 응답에만 영향을 주고 자동으로 사라집니다.</div>
                    </div>
                    <div class="creator-form-section">
                        <label class="required">프롬프트</label>
                        <textarea id="creator-prompt" placeholder="캐릭터의 특징, 성격, 배경, 이야기 등을 작성해주세요." style="height: 400px;"></textarea>
                    </div>
                </div>
            </div>
    ...
    const resetToCreateMode = () => {
        ...
        delete modal.dataset.editId; delete modal.dataset.editPassword; delete modal.dataset.originalIsPublic;
        document.getElementById('creator-image-url').value = '';
        document.getElementById('creator-name').value = '';
        document.getElementById('creator-description').value = '';
        document.getElementById('creator-prompt').value = '';
        document.getElementById('creator-is-public').value = 'false';
        document.getElementById('creator-first-message').value = '';
        document.getElementById('creator-start-setting').value = '';
    ...
    submitBtn.addEventListener('click', async () => {
        ...
        const firstMessageText = document.getElementById('creator-first-message').value.trim();
        const startSettingText = document.getElementById('creator-start-setting').value.trim();
        ...
        try {
            ...
            if (mode === 'create') {
                ...
                if (statusChanged) {
                    const promptForNewPassword = newIsPublic ? `캐릭터를 '공개 플레이'로 전환합니다.\n수정용 새 비밀번호를 설정하세요.` : `캐릭터를 '비공개 플레이'로 전환합니다.\n플레이/수정용 새 비밀번호를 설정하세요.`;
            ...
            if (newIsPublic) {
                finalPromptData = JSON.stringify({ "type": "public", "prompt": promptText, "auth": await cryptoUtils.encryptData(password, "REPPLEY_AUTH_CHECK") });
            } else {
                finalPromptData = JSON.stringify({ "type": "private", "data": await cryptoUtils.encryptData(password, promptText) });
            }
            ...
            const characterData = {
                name,
                description,
                image_url: document.getElementById('creator-image-url').value.trim(),
                is_public: newIsPublic,
                encrypted_prompt: finalPromptData,
                // ✨ [수정] first_message를 JSON 객체로, start_setting을 텍스트로 저장. 비어있으면 null로 저장.
                first_message: firstMessageText ? { greeting: firstMessageText } : null,
                start_setting: startSettingText || null
            };
    ...
    deleteBtn.addEventListener('click', async () => {
        const characterId = modal.dataset.editId;
        if (!characterId) { alert('삭제할 캐릭터 정보가 없습니다.'); return; }
        if (!confirm('정말로 이 캐릭터를 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.')) { return; }
        deleteBtn.disabled = true; submitBtn.disabled = true;
        try {
            ...
            deleteBtn.disabled = false; submitBtn.disabled = false;
        }
    });
  }
